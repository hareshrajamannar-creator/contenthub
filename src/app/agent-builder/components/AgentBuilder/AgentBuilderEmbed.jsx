/**
 * AgentBuilderEmbed.jsx
 *
 * Same logic as AgentBuilder.jsx but without the AppShell wrapper.
 * Designed to be embedded inside birdeyev2 Content Hub views.
 *
 * Key differences vs AgentBuilder:
 *  - No AppShell import / wrapper
 *  - Renders its own embed header (.ab-embed-page / .ab-embed-header)
 *  - onBack prop shows a back button with a configurable backLabel
 */
...
function applyDagreLayout(rawNodes, rawEdges) {
  ...
  g.setGraph({ rankdir: 'TB', ranksep: 32, nodesep: 80, marginx: 40, marginy: 40 });
...
export default function AgentBuilderEmbed({
  appTitle = 'Reviews AI',
  pageTitle = 'Untitled agent',
  activeNavId = 'reviews',
  startSubtitle = 'AI-powered workflow agent',
  defaultNodes = [],
  defaultNodeDetails = {},
            ...
            onChange: (field, value) => {
              ...
              if (field === 'description') {
                setNodeList((prev) => prev.map((n) => {
                  ...
                  return { ...n, data: { ...n.data, subtitle: value || n.data.subtitle } };
                }));
              }
            },
          }}
  ...
  return (
    <div className="ab-embed-page">
      {/* ─── Embed header ─── */}
      <div className="ab-embed-header">
        <div className="ab-embed-header__left">
          {onBack && (
            <button className="ab-embed-back-btn" type="button" onClick={onBack}>
              <span className="material-symbols-outlined">arrow_back</span>
            </button>
          )}
          <span className="ab-embed-header__title">{pageTitle}</span>
        </div>
        <div className="ab-embed-header__right">
          <button
            className="ab-publish-btn"
